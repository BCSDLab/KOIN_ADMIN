...
const actions={
  ...
  deleteCompany(context){
    return new Promise(
      (resolve, reject) =>{
        api.removeCompany(context.state.companyList.id).then(res=>{
          context.state.companyInfo = null;
          resolve(res);
        }, err=>{
          console.log(err);
          reject(err)
    ...
    return new Promise(
      (resolve, reject) => {
        api.getCompany(context.state.companyList[index].id).then(
          res=>{
            console.log(context.state.companyList[index]);
